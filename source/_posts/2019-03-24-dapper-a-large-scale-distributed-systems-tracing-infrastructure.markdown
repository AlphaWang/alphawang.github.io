...
分布式服务的跟踪系统需要记录在一次请求后系统完成的所有工作的信息。举个例子，图-1展示了拥有5台服务器的服务：一个前端服务器A，两个中间层B和C，两个后端服务器D和E。

![dapper-1](/images/post/2019/dapper/dapper-1.png)

*(图-1. 这个路径有用户请求X发起，穿过一个简单的服务系统。用字母标识的节点表示分布式系统中的处理过程)*
